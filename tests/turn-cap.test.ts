...
describe("createTurnCap", () => {
  ...
  it.each(["shouldStopAfterTurn", "finishTurn"] as const)(
    "calls the optional on-cap callback only at the cap via %s",
    (hook) => {
      const contexts: unknown[] = [];
      const cap = createTurnCap(2, (context) => contexts.push(context));
      const finalTurn = { message: { stopReason: "stop", content: [{ type: "text" }] } };
      cap[hook](completedTurn);
      expect(contexts).toEqual([]);
      cap[hook](finalTurn);
      expect(contexts).toEqual([finalTurn]);
    },
  );

  it.each(["shouldStopAfterTurn", "finishTurn"] as const)(
    "does not call on-cap for errors or aborts via %s",
    (hook) => {
      const contexts: unknown[] = [];
      const cap = createTurnCap(1, (context) => contexts.push(context));
      cap[hook](errorTurn);
      cap[hook](abortedTurn);
      expect(contexts).toEqual([]);
      cap[hook](completedTurn);
      expect(contexts).toEqual([completedTurn]);
    },
  );
});
